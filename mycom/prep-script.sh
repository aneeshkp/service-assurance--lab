...
##### - overcloudrc - credentials to login into admin project
##### - openshiftrc - credentials to login into openshift project
...
PUBLIC_SUBNET_NAME=public_network
PUBLIC_NETWORK_POOL_START=10.19.111.152
PUBLIC_NETWORK_POOL_END=10.19.111.199
PUBLIC_NETWORK_GATEWAY=10.19.111.254
PUBLIC_SUBNET_RANGE=10.19.111.0/24

#OpenShift private network details
OCP_NETWORK_NAME=openshift
OCP_NETWORK_RANGE=172.69.3.0/24

#RHEL image details
RHEL_FILE_NAME=rhel-guest-image-7.3-35.x86_64.qcow2 ##change
RHEL_IMAGE_NAME=rhel75a 

#OpenShift instances details
OCP_DOMAIN=mycomlab.com
OCP_BASTION="bastion"
OCP_LB=""
OCP_MASTERS="master1"
OCP_INFRANODES="infranode1"
...
VOLUME_TYPE=nfstype #specify the volume type to user in the docker disks creation
...
##### OpenShift network creation - private network
...
#cinder create --name $HOST-docker ${VOLUME_SIZE}
done
fi
...
do
cinder create --volume_type $VOLUME_TYPE --name $HOST-docker ${VOLUME_SIZE}
#cinder create $VOLUME_TYPE --name $HOST-docker ${VOLUME_SIZE}
